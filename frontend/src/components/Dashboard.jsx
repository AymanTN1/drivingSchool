...
export default function Dashboard({ authData, onLogout }) {
  ...
  const handleScheduleExam = (e) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/assistant/candidates/${candId}/exam`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
  ...
  const handleCompleteLesson = (lessonId, comments, rating) => {
    fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/moniteur/lessons/${lessonId}/complete`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
              ...
              {payrollData.map(m => (
                <div key={m.userId} className="card">
                  ...
                  <button
                    ...
                    onClick={() => {
                      fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/payroll/generate/${m.userId}`, {
                        method: 'POST',
                        headers: {
                          'Authorization': `Bearer ${token}`,
                          'Content-Type': 'application/json'
                      ...
                      <tr key={s.id}>
                        ...
                        <td>
                          {s.status === 'GENERATED' && (
                            <button
                              onClick={() => {
                                fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/payroll/slips/${s.id}/pay`, {
                                  method: 'PUT',
                                  headers: { 'Authorization': `Bearer ${token}` }
                                })
                                  .then(async (res) => {
                          ...
                          <div key={p.id} className="card" style={{ padding: '14px', margin: 0, border: needsAlert ? '1px solid #ef4444' : '1px solid rgba(255,255,255,0.1)', position: 'relative' }}>
                            ...
                            <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginTop: '10px' }}>
                              {col.id !== 'NEW' && (
                                <button 
                                  onClick={() => {
                                    fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/assistant/prospects/${p.id}/status`, {
                                      method: 'PUT',
                  ...
                  onClick={() => {
                    ...
                    scanner.render((decodedText) => {
                      try {
                        ...
                        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8080"}/api/moniteur/lessons/${lessonId}/start`, {
                          method: 'POST',
                          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
