...
import { Monitor, Phone, MapPin, CheckCircle, Clock, AlertTriangle, ArrowRight, User } from 'lucide-react';
...
export default function LandingPage({ onLoginClick, user }) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8080/api/public/pc-posts')
      .then(res => res.json())
      .then(data => {
        if (data.posts) {
          setPcStatus(data);
      ...
      .catch(err => console.log('Backend not started yet, using mock data for landing page.'));
  }, []);
